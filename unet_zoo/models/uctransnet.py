...
from .common_layers import ConfigDict

def get_uctransnet_config():
    config = ConfigDict()
    config.base_channel = 16
    ...
    channel_num_values = [config.base_channel * (2**i) for i in range(4)]
    config.KV_size = sum(channel_num_values)
...
class ConvBatchNorm(nn.Module):
    """(convolution => [BN] => ReLU) for UCTransNet"""
...
class DownBlock(nn.Module):
    """Downscaling with maxpool convolution for UCTransNet"""
...
class CCA(nn.Module):
    """
    CCA Block (Cross Channel Attention) for UCTransNet
    """
...
class UCTransNet(nn.Module):
    def __init__(self, config, in_channels=3, num_classes=1, img_size=224, vis=False, **kwargs):
        super().__init__()
        self.vis = vis
        self.n_channels = in_channels
        self.n_classes = num_classes
        in_channels = config.base_channel
        self.inc = ConvBatchNorm(self.n_channels, in_channels) 
        ...
        self.up4 = UpBlock_attention(in_channels*8 + in_channels*8, in_channels*4, nb_Conv=2) 
        self.up3 = UpBlock_attention(in_channels*4 + in_channels*4, in_channels*2, nb_Conv=2) 
        self.up2 = UpBlock_attention(in_channels*2 + in_channels*2, in_channels, nb_Conv=2) 
        self.up1 = UpBlock_attention(in_channels + in_channels, in_channels, nb_Conv=2) 
        
        self.outc = nn.Conv2d(in_channels, num_classes, kernel_size=(1,1), stride=(1,1))
    ...
    def forward(self, x):
        ...
        x1_refined, x2_refined, x3_refined, x4_refined, att_weights = self.mtc(x1,x2,x3,x4)

        x = self.up4(x5, x4_refined) 
        x = self.up3(x, x3_refined) 
        x = self.up2(x, x2_refined) 
        x = self.up1(x, x1_refined) 
